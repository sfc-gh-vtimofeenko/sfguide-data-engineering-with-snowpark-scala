...
object Step07_UpdateDailyCityMetricsProcedure extends WithLogging with WithSession with WithWHResize {
  ...
  def mergeDailyCityMetrics(session: Session): Try[Unit] = withWHResize {
    ...
    /** This allows printing the dataframe while it's being transformed so longer chains of methods don't need to be
      * interrupted
      */
    implicit class PrintableDataframe(df: DataFrame) {
      def printAndPassDf(dfName: String): DataFrame = {
        logger info s"$dfName:"
    ...
    /** Prints the tree structure of the dataframe with a header */
    def showDataFrame(df: DataFrame, dfName: String): Unit = {
      logger info s"$dfName:"
      df.schema printTreeString
    }
    ...
    val weatherJoined = weather
      ...
      .join(
        countries,
        (weather col "COUNTRY" equal_to countries.col("ISO_COUNTRY"))
          and
            (weather col "CITY_NAME" equal_to countries.col("CITY"))
      )
    ...
    val dailyCityMetricsStg: DataFrame = orders
      ...
      .drop(weatherAgg.col("DATE"), weatherAgg.col("CITY_NAME"), weatherAgg.col("COUNTRY_DESC"))
    ...
    dcm
      .merge(
        ...
        (Seq("DATE", "CITY_NAME", "COUNTRY_DESC")
          .map(colname => dcm col colname equal_to dailyCityMetricsStg.col(colname)))
