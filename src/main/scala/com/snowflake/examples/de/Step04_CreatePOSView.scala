...
object Step04_CreatePOSView extends WithLocalSession {
  ...
  private def createPOSView(session: Session): Try[Unit] = Try {
    val orderDetail = session
      .table("RAW_POS.ORDER_DETAIL")
      .select(
        Seq(
          col("ORDER_DETAIL_ID"),
          col("LINE_NUMBER"),
    ...
    val orderHeader = session
      .table("RAW_POS.ORDER_HEADER")
      .select(
        Seq(
          col("ORDER_ID"),
          col("TRUCK_ID"),
    ...
    val franchise = session
      .table("RAW_POS.FRANCHISE")
      .select(
        Seq(
          col("FRANCHISE_ID"),
    ...
    val location = session.table("RAW_POS.LOCATION").select(Seq(col("LOCATION_ID")))

    // Perform joins
    val tWithF = truck.join(franchise, usingColumn = "FRANCHISE_ID")

    // UsingColumn keeps the code DRY
    val ohWtAndL = orderHeader.join(tWithF, usingColumn = "TRUCK_ID").join(location, usingColumn = "LOCATION_ID")

    orderDetail
      .join(ohWtAndL, usingColumn = "ORDER_ID")
      .join(menu, usingColumn = "MENU_ITEM_ID")
      .select(
        Seq(
  ...
  private def createPOSViewStream(session: Session): Try[Unit] = Try {
    session
      .sql("CREATE OR REPLACE STREAM POS_FLATTENED_V_STREAM ON VIEW POS_FLATTENED_V SHOW_INITIAL_ROWS = TRUE")
      .collect()
  }
  ...
  def execute(session: Session) = {
    ...
    createPOSView(session) match {
      case Success(_) => logger.info("Created POS View")
    ...
    createPOSViewStream(session) match {
      case Success(_) => logger info "Created POS view stream"
      case Failure(exception) => {
